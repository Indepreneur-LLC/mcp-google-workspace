...
async def list_drive_files(
    ...
    try:
        if not user_email: raise ValueError("User email must be provided.") # Use standard error

        # Get service using gauth helper
        service = await asyncio.to_thread(gauth.get_google_service, 'drive', 'v3', user_email)
        if service is None:
            # This case should ideally be handled by get_google_service raising AuthenticationError
            logger.error(f"Failed to obtain Google Drive service for user {user_email}. Credentials might be missing or invalid.")
            raise gauth.AuthenticationError(f"Failed to obtain Google Drive service for user {user_email}. Authentication may be required or invalid.")
        ...
        # Return the file list dict
        return files

    except gauth.AuthenticationError as auth_error:
        # Re-raise the specific authentication error for the MCP server to handle
        logger.warning(f"Authentication error in list_drive_files for user {user_email}: {auth_error}")
        raise auth_error # Propagate the error
    except HttpError as e:
        logger.error(f"Google API HTTP error in list_drive_files for user {user_email}: {e}", exc_info=True)
        ...
        logger.error(f"Unexpected error in list_drive_files for user {user_email}: {e}", exc_info=True)
        # Raise a standard runtime error
        raise RuntimeError(f"Failed to list Drive files for {user_email}. Reason: {e}")
...
async def get_drive_file_metadata(
    ...
    try:
        if not user_email: raise ValueError("User email must be provided.")
        if not file_id: raise ValueError("file_id must be provided.")

        # Get service using gauth helper
        service = await asyncio.to_thread(gauth.get_google_service, 'drive', 'v3', user_email)
        if service is None:
            logger.error(f"Failed to obtain Google Drive service for user {user_email}.")
            raise gauth.AuthenticationError(f"Failed to obtain Google Drive service for user {user_email}. Authentication may be required or invalid.")
        ...
        logger.warning(f"Authentication error in get_drive_file_metadata for user {user_email}, file {file_id}: {auth_error}")
        raise auth_error # Propagate
    except HttpError as e:
        if e.resp.status == 404:
            logger.warning(f"File {file_id} not found for metadata request by user {user_email}: {e}")
            # Raise a standard FileNotFoundError
            raise FileNotFoundError(f"File with ID '{file_id}' not found.")
        else:
            logger.error(f"Google API HTTP error in get_drive_file_metadata for user {user_email}, file {file_id}: {e}", exc_info=True)
            # Raise a standard runtime error
            raise RuntimeError(f"Google API Error: {e.resp.status} {e.reason}. Details: {e.content.decode()}")
    except Exception as e:
        logger.error(f"Unexpected error in get_drive_file_metadata for user {user_email}, file {file_id}: {e}", exc_info=True)
        # Raise a standard runtime error
        raise RuntimeError(f"Failed to get metadata for file {file_id} for user {user_email}. Reason: {e}")

async def download_drive_file(file_id: str, user_email: str) -> dict:
    """Downloads a Google Drive file and returns it as a dict with blob data."""
    try:
        if not user_email: raise ValueError("User email must be provided.")
        if not file_id: raise ValueError("file_id must be provided.")
        ...
        try:
            ...
            if meta_http_error.resp.status == 404:
                logger.warning(f"File {file_id} not found for metadata check before download by user {user_email}: {meta_http_error}")
            ...
            # Handle 403 for permission issues
                 ...
                 logger.warning(f"Permission denied getting metadata for file {file_id} for user {user_email}: {meta_http_error}")
                ...
                logger.error(f"Google API HTTP error getting metadata before download for user {user_email}, file {file_id}: {meta_http_error}", exc_info=True)
            ...
            logger.error(f"Unexpected error getting metadata before download for user {user_email}, file {file_id}: {meta_e}", exc_info=True)
        ...
        if file_content is None:
            ...
            logger.error(f"Drive API download returned None unexpectedly for file {file_id}, user {user_email}")
        ...
        try:
            file_content_b64 = await asyncio.to_thread(lambda: base64.b64encode(file_content).decode('utf-8'))
             ...
             logger.error(f"Base64 encoding error for downloaded file {file_id}, user {user_email}: {encode_error}", exc_info=True)
             raise RuntimeError(f"Failed to encode downloaded file data. Reason: {encode_error}")


        # Construct the resource dict
        resource_dict = {
        ...
        logger.warning(f"Authentication error in download_drive_file for user {user_email}, file {file_id}: {auth_error}")
        raise auth_error # Propagate
        ...
        if e.resp.status == 404: # Should have been caught by metadata check, but handle defensively
            logger.warning(f"File {file_id} not found during download attempt by user {user_email}: {e}")
            raise FileNotFoundError(f"File with ID '{file_id}' not found during download.")
        elif e.resp.status == 403:
            logger.warning(f"Permission denied downloading file {file_id} for user {user_email}: {e}")
            raise PermissionError(f"Permission denied to download file '{file_id}'.")
        else:
            logger.error(f"Google API HTTP error during download for user {user_email}, file {file_id}: {e}", exc_info=True)
            raise RuntimeError(f"Google API Error downloading file: {e.resp.status} {e.reason}. Details: {e.content.decode()}")
    except Exception as e:
        logger.error(f"Unexpected error in download_drive_file for user {user_email}, file {file_id}: {e}", exc_info=True)
...
async def upload_drive_file(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to decode base64 content for upload '{file_name}' (user: {user_email}): {decode_error}", exc_info=True)
        ...
        # Return the upload response dict
        return upload_response

    except gauth.AuthenticationError as auth_error:
        logger.warning(f"Authentication error in upload_drive_file for user {user_email}, file '{file_name}': {auth_error}")
        raise auth_error # Propagate
        ...
        if e.resp.status == 404 and folder_id:
            logger.warning(f"Target folder {folder_id} not found for upload by user {user_email}: {e}")
            # Raise FileNotFoundError for the folder
            raise FileNotFoundError(f"Target folder with ID '{folder_id}' not found.")
        # Handle 403 for permission issues
        elif e.resp.status == 403:
            logger.warning(f"Permission denied uploading file '{file_name}' for user {user_email} (folder: {folder_id}): {e}")
            # Raise PermissionError
            raise PermissionError(f"Permission denied to upload file '{file_name}' (check folder permissions).")
        else:
            logger.error(f"Google API HTTP error in upload_drive_file for user {user_email}, file '{file_name}': {e}", exc_info=True)
        ...
        logger.error(f"Unexpected error in upload_drive_file for user {user_email}, file '{file_name}': {e}", exc_info=True)
        # Raise standard RuntimeError
